...
class ScanActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        const val EXTRA_INITIAL_VIEW_MODE = "initialViewMode"
    ...
    /** Backup of pre-dewow values, restored if the dewow switch is turned back off. */
    private var preDewowValues: Array<FloatArray>? = null

    /** True when launched from the "GPR Taraması" menu flow (see GridScanActivity.EXTRA_GPR_MODE). */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isGprMode) {
            // GPR mode opens straight into the analysis tools panel
            // (Fonksiyon/Renk Skalası/Eşik/Dewow/CSV) since the whole
            // point of the GPR flow is "scan, then immediately apply
            // analysis techniques to the result."
            binding.interpolationPanel.visibility = android.view.View.VISIBLE
        }

        // If launched from a handoff that requests a specific starting
        // tab (e.g. GridScanActivity's manual-scan workflow opens
        // straight into 3D Hacimsel for immediate analysis), honor it.
        intent.getStringExtra(EXTRA_INITIAL_VIEW_MODE)
            ?.let { runCatching { com.arkeosar.groundscan.render.ViewMode.valueOf(it) }.getOrNull() }
            ?.let { setViewMode(it) }

        if (openFilePath != null) {
            binding.statusText.text = (if (isGprMode) getString(com.arkeosar.groundscan.R.string.menu_gpr) + " · " else "") + File(openFilePath).name
        } else {
    ...
    private fun setViewMode(mode: com.arkeosar.groundscan.render.ViewMode) {
        ...
        // In GPR mode, the "this is schematic, not a real measurement"
        // disclaimer stays visible on every tab, not just Hacimsel -
        // since the whole GPR-framed flow carries that caveat, not just
        // its volumetric view.
        binding.schematicDisclaimer.visibility =
            if (isGprMode || mode == com.arkeosar.groundscan.render.ViewMode.VOLUMETRIC_3D) android.view.View.VISIBLE
    ...
    /**
     * Wires up the "Dewow Filtresi Uygula" switch: applying it runs
     * [com.arkeosar.groundscan.data.DewowFilter] over the current grid
     * (after backing up the original values), turning it back off
     * restores the backed-up originals - so dewow is a reversible
     * toggle, not a one-way destructive edit.
     */
    private fun setupDewowToggle() {
        binding.switchDewow.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                preDewowValues = Array(grid.rows) { row -> FloatArray(grid.columns) { col -> grid.pointAt(row, col).value } }
                com.arkeosar.groundscan.data.DewowFilter.applyInPlace(grid)
            } else {
                val backup = preDewowValues
                if (backup != null) {
                    for (row in 0 until grid.rows) {
                        for (col in 0 until grid.columns) {
                            val v = backup[row][col]
                            if (!v.isNaN()) grid.pointAt(row, col).value = v
                        }
                    }
                }
                preDewowValues = null
    ...
    private fun setupThresholdSlider() {
        binding.seekThreshold.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, value: Int, fromUser: Boolean) {
                ...
                renderer.volumetricThreshold = thresholdFraction
                renderer.invalidateMesh()
            }
    ...
    private fun setupToolbars() {
        ...
        binding.btnOpenBScan.setOnClickListener { openBScan() }
    }

    /**
     * Saves the current scan to a temporary file and opens
     * [BScanActivity] on it - lets the person view the same data as a
     * GPR-style cross-section without leaving the analysis flow.
     */
    private fun openBScan() {
        val dir = File(filesDir, "scans").also { it.mkdirs() }
        val file = File(dir, "bscan_temp_${System.currentTimeMillis()}.asgs")
        ArkeoSarFile.save(grid, file, metadata = mapOf("source" to "ScanActivity-BScanHandoff"))
        startActivity(
            android.content.Intent(this, BScanActivity::class.java)
                .putExtra(BScanActivity.EXTRA_FILE_PATH, file.absolutePath)
        )
    }
