...
class HeightmapMesh(
    ...
    private val flattenToTopDown: Boolean = false
) {
    ...
    init {
        ...
        // For ViewMode.TOP_DOWN_2D, the surface is flattened down to a
        // single plane (Y pinned to the top of where the relief box
        // would otherwise extend) while keeping the same X/Z footprint
        // and reference frame - this mirrors the reference app's 2D/3D
        // toggle, which collapses the surface's height rather than just
        // changing the camera angle, so the bounding box and grid lines
        // stay visually consistent between modes.
        val flattenedY = if (flattenToTopDown) (gridSpan * maxHeightFraction) / 2f else null
        ...
        for (row in 0 until outRows) {
            for (col in 0 until outCols) {
                ...
                val y = flattenedY ?: ((h - heightOffset) * heightScale)
