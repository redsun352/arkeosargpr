...
class HeightmapRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    @Volatile var volumetricThreshold: Float = 0.45f
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        if (meshDirty) {
            mesh = HeightmapMesh(currentGrid, flattenToTopDown = viewMode == ViewMode.TOP_DOWN_2D)
            volumetricMesh = VolumetricMesh(currentGrid, thresholdFraction = volumetricThreshold)
     ...
     * and yaw ([rotationY]). [ViewMode.TOP_DOWN_2D] flattens the mesh
     * itself (see [HeightmapMesh]'s flattenToTopDown) rather than
     * forcing the camera angle, so the person can still orbit around
     * the flattened plane and see it sitting inside the reference box -
     * matching the reference app's 2D/3D toggle behavior.
     */
    ...
    private fun drawVolumetric() {
        ...
        if (currentVolumetric.triangleCount == 0) return

        GLES20.glEnable(GLES20.GL_BLEND)
        GLES20.glBlendFunc(GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA)
        // Depth writes off (but depth test on) so overlapping translucent
        // wall segments blend with each other instead of occluding by depth.
        GLES20.glDepthMask(false)
        // The isosurface walls are open (no back-face culling), since
        // they're meant to be seen from both inside and outside the
        // tapering shape.
        ...
        GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, currentVolumetric.triangleCount * 3)
