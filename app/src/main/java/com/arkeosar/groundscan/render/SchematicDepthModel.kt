...
object SchematicDepthModel {
    ...
    /**
     * Inverse-cube-style falloff: strength decays monotonically as
     * [sliceDepth] increases below the surface, modeling how a dipole's
     * field weakens with distance (B ∝ 1/r³ in the standard near-field
     * approximation). [assumedDepth] sets the *characteristic* decay
     * distance - at sliceDepth == assumedDepth, strength has dropped to
     * 1/8 of the surface value (since (d / (d+d))³ = (1/2)³ = 1/8) -
     * rather than literally being "the depth of the source" in a
     * precise inversion sense.
     *
     * This form (referenceDistance / (referenceDistance + sliceDepth))
     * is deliberately singularity-free and strictly decreasing for all
     * sliceDepth >= 0, unlike a formula that divides by
     * (assumedDepth - sliceDepth) - that version blows up and even
     * reverses direction as sliceDepth approaches/exceeds assumedDepth,
     * which produced nonsensical (growing-with-depth, then exploding)
     * volumes here previously.
     */
    private fun falloff(surfaceValue: Float, sliceDepth: Double, assumedDepth: Double): Float {
        ...
        val ratio = (referenceDistance / (referenceDistance + sliceDepth.coerceAtLeast(0.0))).pow(3)
