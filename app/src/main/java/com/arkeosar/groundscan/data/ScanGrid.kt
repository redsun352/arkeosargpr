...
class ScanGrid(
    ...
    /**
     * When true, the very first line's sweep starts from the opposite
     * physical side (e.g. for COLUMN_MAJOR, the first column is walked
     * bottom-to-top instead of top-to-bottom; subsequent columns still
     * alternate normally via [zigzag]). This implements the "Sağdan /
     * Soldan" starting-direction choice on the scan area setup screen -
     * it only flips where the sweep *begins*, not the overall zigzag
     * pattern.
     */
    val startFromOpposite: Boolean = false
) {
     ...
     * ROW_MAJOR, or a column for COLUMN_MAJOR. [startFromOpposite] flips
     * every line's direction (equivalent to flipping which side line 0
     * starts from), since the zigzag alternation is relative to line 0.
     */
    private fun physicalSecondary(line: Int, scanOrderIndex: Int, secondaryLength: Int): Int {
        if (!zigzag) return if (startFromOpposite) (secondaryLength - 1 - scanOrderIndex) else scanOrderIndex
        val lineIsReversed = (line % 2 == 1) != startFromOpposite
    ...
    fun isCurrentLineReversed(): Boolean {
        ...
        return zigzag && (line % 2 == 1) != startFromOpposite
    }
