 ...
 *     Running the calibration screen ([com.arkeosar.groundscan.ui.CalibrationActivity])
 *     before scanning corrects for a constant ("hard-iron") part of this
 *     interference - see [MagnetometerCalibration].
...
class InternalSensorSource(private val context: Context) : ScanDataSource {
    ...
    private val calibration = MagnetometerCalibration(context)
    ...
    override fun start(
        ...
        val sensorListener = object : SensorEventListener {
            override fun onSensorChanged(event: SensorEvent) {
                ...
                // Apply hard-iron bias correction if the person has run
                // the calibration screen; otherwise this is a no-op
                // (bias defaults to zero), so calibration is optional,
                // not required, to use the sensor.
                val (x, y, z) = calibration.apply(rawX, rawY, rawZ)
